...
const AddBlog = () => {
    ...
    const uploadFile = (blogFile) => {
        setFileLoading(true);
        if (!blogFile) return;
        const currentDate = new Date();
        const uniqueFileName = `${currentDate.getTime()}_${blogFile?.name}`;
        const imageRef = ref(storage, `blogFile/${uniqueFileName}`);
        uploadBytes(imageRef, blogFile).then((snapshot) => {
    ...
    return (
        <main className='min-h-screen lg:container py-4 px-4 '>
            ...
            <Form onSubmit={handleSubmit} className="w-full bg_white rounded-3 shadow-md p-4">
                ...
                <Form.Label className="plusJakara_semibold text_dark ms-4">Blog Caption</Form.Label>
                <div className="px-3">
