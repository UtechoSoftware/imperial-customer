...
const AddQuestion = () => {
  ...
  useEffect(() => {
    ...
    const initialQuestions = Array.from(
      { length: totalQuestionsFromLocalStorage },
      (_, index) => ({
        title: "",
        options: [""],
        answers: "",
  ...
  //         console.log(updatedQuestions[questionIndex].options.push(''),"test3")
  ...
  const handleAddOption = (questionIndex) => {
    setQuestions((prevQuestions) => {
      ...
      // Check if there's any empty option
      const hasEmptyOption = options.includes("");

      if (!hasEmptyOption) {
        updatedQuestions[questionIndex].options.push("");
      } else {
        message.error("Please fill the options first");
      }
        ...
        <div className="flex flex-wrap gap-2 mb-3 items-center w-full">
          {Array.from({ length: totalQuestions }, (_, index) => (
            <button
              key={index}
              type="button"
              className={`px-4 py-2 rounded-3 plusJakara_bold ${activeQuestionIndex === index
        ...
        {activeQuestionIndex < totalQuestions && (
          <QuestionItem
            ...
            handleRemoveOption={(optionIndex) =>
              handleRemoveOption(activeQuestionIndex, optionIndex)
            }
          />
        )}
        {console.log(questions[activeQuestionIndex], "test1")}
